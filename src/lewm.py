...
class LeWMDataset(Dataset):
  ...
  def __init__(self, temporal_dataset: TemporalSequenceDataset, *, image_size: int, frame_skip: int, action_type: str, action_encoding: str | None = None, num_actions: int | None = None, action_dim: int | None = None,
               normalization: str = "imagenet", interpolation: str = "bilinear",
               ):
    ...
    interpolation_map = {"bilinear": InterpolationMode.BILINEAR, "nearest": InterpolationMode.NEAREST, }
    ...
    if action_type == "discrete":
      ...
      if action_encoding not in {"one_hot", "centered_binary"}:
        raise ValueError("Discrete LeWM actions require 'one_hot' or 'centered_binary' encoding")
  ...
  def _extract_pixels(observation: Any) -> np.ndarray:
    ...
    raise TypeError("LeWMDataset expected an RGB ndarray or a dict containing 'pixels', "
  ...
  def _process_frame(self, observation: Any) -> torch.Tensor:
    ...
    x = TF.resize(x, [self.image_size, self.image_size], interpolation=self.interpolation, antialias=(self.interpolation == InterpolationMode.BILINEAR), )
  ...
  def _encode_action_chunk(self, chunk: list[Any]) -> torch.Tensor:
    if len(chunk) != self.frame_skip:
      raise ValueError(f"Vanilla LeWM requires {self.frame_skip} actions per chunk; "
                       f"got {len(chunk)}")
    ...
    vectors = [torch.as_tensor(np.asarray(action), dtype=torch.float32).reshape(-1) for action in chunk]
    encoded = torch.cat(vectors)
    if encoded.numel() != self.action_input_dim:
      raise ValueError(f"Continuous action chunk has {encoded.numel()} values; "
  ...
  def __getitem__(self, index: int) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
      ...
      "action": torch.stack([self._encode_action_chunk(chunk) for chunk in sample.action_chunks]),
      "reward": torch.tensor([sum(chunk) for chunk in sample.reward_chunks], dtype=torch.float32),
...
def build_lewm(cfg: DictConfig, *, action_input_dim: int, history_size: int, ) -> tuple[JEPA, SIGReg]:
  encoder = vit_hf(size=cfg.encoder.size, patch_size=cfg.encoder.patch_size, image_size=cfg.image_size, pretrained=cfg.encoder.pretrained, use_mask_token=False, )

  predictor = ARPredictor(num_frames=history_size, input_dim=cfg.embed_dim, hidden_dim=cfg.embed_dim, output_dim=cfg.embed_dim, depth=cfg.predictor.depth, heads=cfg.predictor.heads, mlp_dim=cfg.predictor.mlp_dim,
                          dim_head=cfg.predictor.dim_head, dropout=cfg.predictor.dropout, emb_dropout=cfg.predictor.emb_dropout,
                          )

  model = JEPA(encoder=encoder, predictor=predictor, action_encoder=Embedder(input_dim=action_input_dim, emb_dim=cfg.embed_dim),
               projector=MLP(input_dim=cfg.embed_dim, hidden_dim=cfg.projector.hidden_dim, output_dim=cfg.embed_dim, norm_fn=nn.BatchNorm1d,
                             ), pred_proj=MLP(input_dim=cfg.embed_dim, hidden_dim=cfg.projector.hidden_dim, output_dim=cfg.embed_dim, norm_fn=nn.BatchNorm1d,
                                              ),
               )
...
def compute_lewm_loss(model: JEPA, sigreg: SIGReg, batch: dict[str, torch.Tensor], *, history_size: int, sigreg_weight: float, device: torch.device, ):
  ...
  predicted_emb = model.predict(emb[:, :history_size], act_emb[:, :history_size], )
  target_emb = emb[:, 1:]
