...
"How do I interact with a world?"

        ↓

Trajectory
"What actually happened at every raw step?"

        ↓

TemporalSequenceDataset
"What time scale should the world model see?"
(frame_skip lives here)

        ↓

LeWMDataset
"How does vanilla LeWM want observations/actions represented?"

        ↓

JEPA
"Can I predict the next latent state from current latent + action?"
"""
...
class Trajectory:
  ...
  def append_step(self, *, action: Any, reward: float, next_observation: Any, terminated: bool, truncated: bool, info: dict[str, Any], ) -> None:
  ...
  def validate(self) -> None:
    ...
    if len(self.observations) != n + 1:
      raise ValueError("Trajectory invariant violated: expected "
                       "len(observations) == len(actions) + 1")

    for name, values in {"rewards": self.rewards, "terminated": self.terminated, "truncated": self.truncated, "infos": self.infos, }.items():
      if len(values) != n:
        raise ValueError(f"Trajectory invariant violated: len({name})={len(values)}, "
                         f"len(actions)={n}")
...
def collect_trajectories(env: EnvironmentInterface[Any, Any], *, num_env_steps: int, seed: int | None = None, policy: ActionPolicy | None = None, ) -> list[Trajectory]:
  ...
  episode_id = 0
  trajectory = Trajectory(observations=[deepcopy(observation)], metadata={"episode_id": episode_id, "reset_info": deepcopy(reset_info), "collection_cutoff": False, }, )

  for global_step in range(num_env_steps):
    action = policy(env, observation)
    result = env.step(action)

    trajectory.append_step(action=deepcopy(action), reward=result.reward, next_observation=deepcopy(result.observation), terminated=result.terminated, truncated=result.truncated, info=deepcopy(result.info), )
...
def split_trajectories(trajectories: list[Trajectory], *, train_fraction: float, seed: int, ) -> tuple[list[Trajectory], list[Trajectory]]:
  ...
  if split == 0 or split == len(indices):
    raise ValueError("Not enough trajectories for a non-empty train/validation split. "
...
class TemporalSequenceDataset(Dataset):
  ...
  def __init__(self, trajectories: list[Trajectory], *, history_size: int, frame_skip: int, window_stride: int | None = None, allow_partial_final_chunk: bool = False, ):
    if history_size < 1:
  ...
  def __getitem__(self, index: int) -> TemporalSample:
    ...
    return TemporalSample(observations=observations, action_chunks=action_chunks, reward_chunks=reward_chunks, terminated=terminated, truncated=truncated, infos=info_chunks, metadata={
