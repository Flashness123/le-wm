...
ACTION_DIM = 1
ACTION_INPUT_DIM = FRAME_SKIP * ACTION_DIM
...
def build_lewm() -> JEPA:
    ...
    #      +1 = right
    #
    # So the raw action dimension is 1.
    #
    # We still use the ORIGINAL LeWM Embedder.
    # --------------------------------------------------------
...
def main():
    ...
    print()
    print("=" * 70)
    print("Visual CartPole LeWM training")
    print("=" * 70)
    ...
    # ========================================================
    # Collect visual CartPole experience
    # ========================================================

    print()
    print(
        f"Collecting {NUM_TRANSITIONS} visual transitions..."
    )
    ...
    train_dataset = LeWMPixelSequenceDataset(
        ...
        image_size=IMAGE_SIZE,
        action_encoding="binary_centered",
    )
    ...
    print("pixels:", sample["pixels"].shape)
    print("actions:", sample["action"].shape)
    print("actions:")
    print(sample["action"])
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(
            "    WM metrics | "
            f"pred={world_metrics['final_prediction_mse']:.4f} | "
        ...
        print(
            "               "
            f"opposite-action={world_metrics['counterfactual_mse']:.4f} | "
            f"opposite gain={100 * world_metrics['counterfactual_gain']:.1f}% | "
            f"beats opposite={100 * world_metrics['correct_beats_counterfactual']:.1f}%"
        )
        # ----------------------------------------------------
