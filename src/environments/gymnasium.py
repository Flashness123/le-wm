...
class GymnasiumEnvironment(EnvironmentInterface[Any, Any]):
  ...
  def __init__(self, env_id: str, observation_mode: str = "state", **env_kwargs: Any):
    ...
    if observation_mode == "state":
      ...
      self.env = gym.wrappers.AddRenderObservation(env, render_only=(observation_mode == "pixels"), )
    else:
      raise ValueError("observation_mode must be 'state', 'pixels', or 'pixels_with_state'")
  ...
  def step(self, action: Any) -> StepResult[Any]:
    ...
    return StepResult(observation=observation, reward=float(reward), terminated=bool(terminated), truncated=bool(truncated), info=info, )
