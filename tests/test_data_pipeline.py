...
def make_trajectory(num_steps: int, *, terminal: bool = True) -> Trajectory:
  ...
  return Trajectory(observations=observations, actions=actions, rewards=rewards, terminated=terminated, truncated=truncated, infos=infos, )
...
class TemporalDatasetTests(unittest.TestCase):
  ...
  def test_frame_skip_is_applied_after_collection(self):
    dataset = TemporalSequenceDataset([make_trajectory(8)], history_size=2, frame_skip=2, )
    sample = dataset[0]
  ...
  def test_partial_terminal_chunk_is_preserved_when_enabled(self):
    dataset = TemporalSequenceDataset([make_trajectory(5, terminal=True)], history_size=2, frame_skip=3, allow_partial_final_chunk=True, )
    sample = dataset[0]
  ...
  def test_partial_terminal_chunk_is_hidden_from_fixed_width_model(self):
    dataset = TemporalSequenceDataset([make_trajectory(5, terminal=True)], history_size=2, frame_skip=3, allow_partial_final_chunk=False, )
